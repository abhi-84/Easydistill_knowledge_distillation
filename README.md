...
9. Save model state as checkpoints to be used
10. Test the saved model checkpoint using file test_model_interactive.py
11. Download the 7B teacher model from the Hugging Face Hub
12. Split the dataset-100k into multiple chunks of 20K each to save memory space, split_dataset.py
13. Create 5 config files for each database chunk split_json.py
14. Use config file for each chunk and fo inference to generate logits for each chunk
...
15. Merging all logits into a single logit file merge_logit.py
16. Perform the student model training using logit file and stage2.json config file. Update the stage2.json
file with correct configuration values, logits.json path, student model and teacher model path and path for result
...
17. Test the trained student model using test_model_interactive.py
18. Run Flask server, host the front-end UI and host the model
